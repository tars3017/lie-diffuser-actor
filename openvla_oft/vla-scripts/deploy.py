...
class OpenVLAServer:
    def __init__(self, cfg) -> Path:
        ...
        # Load noisy action projector if using diffusion (SE(3) has it built-in)
    ...
    def get_server_action(self, payload: Dict[str, Any]) -> str:
        try:
            ...
            action = get_vla_action(
                self.cfg, self.vla, self.processor, observation, instruction, action_head=self.action_head, proprio_projector=self.proprio_projector, noisy_action_projector=self.noisy_action_projector, use_film=self.cfg.use_film,
            )
