...
from prismatic.models.action_heads import DiffusionActionHead, L1RegressionActionHead, SE3ScoreMatchingActionHead
...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    If cfg.base_vla_path is set, loads the base VLA model from that path and
    dataset statistics from cfg.pretrained_checkpoint. This is needed when the
    VLM was frozen during training and only the action head / proprio projector
    were saved to the checkpoint directory.
    ...
    # Determine where to load the VLA model weights from
    base_vla_path = getattr(cfg, "base_vla_path", None)
    vla_model_path = base_vla_path if base_vla_path else cfg.pretrained_checkpoint
    ...
    if not model_is_on_hf_hub(vla_model_path):
        ...
        update_auto_map(vla_model_path)
        check_model_logic_mismatch(vla_model_path)
    ...
    if base_vla_path:
        print(f"Loaded base VLA from: {base_vla_path}")
        print(f"Will load component checkpoints from: {cfg.pretrained_checkpoint}")
    ...
    # Load dataset stats from the fine-tuned checkpoint (not the base model)
...
def get_processor(cfg: Any) -> AutoProcessor:
    ...
    """
    # Try loading from pretrained_checkpoint first; fall back to base_vla_path
    try:
        return AutoProcessor.from_pretrained(cfg.pretrained_checkpoint, trust_remote_code=True)
    except Exception:
        base_vla_path = getattr(cfg, "base_vla_path", None)
        if base_vla_path:
            print(f"Processor not found in checkpoint, loading from base_vla_path: {base_vla_path}")
            return AutoProcessor.from_pretrained(base_vla_path, trust_remote_code=True)
        raise
...
def get_action_head(cfg: Any, llm_dim: int) -> Union[L1RegressionActionHead, DiffusionActionHead, SE3ScoreMatchingActionHead]:
    """
    ...
    Returns:
        The initialized action head (L1RegressionActionHead, DiffusionActionHead, or SE3ScoreMatchingActionHead)

    Raises:
        AssertionError: If more than one head type is specified
    """
    num_active = sum([cfg.use_l1_regression, cfg.use_diffusion, getattr(cfg, "use_se3_score_matching", False)])
    assert num_active <= 1, "At most one action head type (l1_regression, diffusion, se3_score_matching) can be active!"
    ...
    elif getattr(cfg, "use_se3_score_matching", False):
        action_head = SE3ScoreMatchingActionHead(
            input_dim=llm_dim, hidden_dim=llm_dim, action_dim=ACTION_DIM,
            num_steps_train=cfg.num_se3_steps_train,
            lie_group=getattr(cfg, "score_matching_lie_group", True),
        )
        # Allow overriding inference steps (e.g., fewer for faster deployment)
        action_head.num_inference_steps = getattr(cfg, "num_se3_steps_inference", cfg.num_se3_steps_train)
    else:
        raise ValueError("One of use_l1_regression, use_diffusion, or use_se3_score_matching must be True")
