...
class Encoder(nn.Module):
    ...
    def run_fps(self, context_features, context_pos):
        ...
        # Sample points with FPS. The dgl wheel we depend on doesn't ship a
        # CUDA kernel for FarthestPointSampler, so we round-trip through CPU
        # and bring the resulting indices back to the input's device. The
        # tensors here are tiny (a few hundred 192-D points), so the offload
        # is negligible next to the rest of the forward pass.
