...
from lda.eval.evaluate_utils import convert_action
from lda.utils.calvin import relative_to_absolute
...
class DiffusionModel(CalvinBaseModel):
    ...
    def load_pretrained_weights(self, state_dict=None):
        ...
        for key in state_dict:
            ...
            # For the torch_geometric==2.6.1
            # if _key.endswith('lin_src.weight'):
            #     lin_src = state_dict[key]
            #     lin_dst_key = _key.replace('lin_src.weight', 'lin_dst.weight')
            #     if (key[:7] + lin_dst_key) in state_dict:
            #         lin_dst = state_dict[key[:7] + lin_dst_key]
            #         model_weights[_key.replace('lin_src.weight', 'lin.weight')] = (lin_src + lin_dst) / 2
            #     continue
            # elif _key.endswith('lin_dst.weight'):
            #     continue
    ...
    def step(self, obs, instruction, all_joint_coords):
        """
        Args:
        ...
        sample = {}
        sample['rgbs'] = rgbs.float()
        sample['pcds'] = pcds.float()
        sample['instr'] = instruction.float()
        sample['joints_coords'] = torch.as_tensor(all_joint_coords).to(device).unsqueeze(0).float()
        ...
        # gripper_zeros = torch.zeros(trajectory.shape[0], trajectory.shape[1], 1, device=trajectory.device, dtype=trajectory.dtype)
        # trajectory = torch.cat([trajectory, gripper_zeros], dim=-1)
