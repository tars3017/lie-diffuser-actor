...
from lda.eval.evaluate_model import create_model
from lda.eval.evaluate_utils import (
...
from lda.eval.multistep_sequences import get_sequences
from lda.eval.evaluate_utils import get_env

logger = logging.getLogger(__name__)

EP_LEN = 60
# Override with NUM_SEQUENCES=<n> for smoke runs / debugging. Default
# 1000 matches the paper-table evaluation protocol.
...
class Arguments(tap.Tap):
    ...
    # Ablation flags (must match the training config that produced the ckpt
    # so DiffuserActor instantiates the correct branches).
    use_gat: int = 1
    diffusion_space: str = "lie"            # "lie" or "euclidean"
    loss_formulation: str = "new"            # "new" or "old"
...
def rollout(env, model, task_oracle, subtask, lang_annotation):
    ...
    for step in pbar:
        ...
        for idx in joint_indices:
            ...
            pos = link_state[0]  # (x, y, z)
            orn = link_state[1]  # quaternion (x, y, z, w)
            joint_states.append(np.concatenate([np.array(pos), np.array(orn)], axis=0))  # (7,)
        all_joint_coords = torch.from_numpy(np.stack(joint_states, axis=0))

        with torch.cuda.amp.autocast(enabled=False):
            trajectory = model.step(obs, lang_embeddings, all_joint_coords)
...
if __name__ == "__main__":
    # Eval scripts feed the full training YAML via `to_cli_args`, which emits
    # training-only flags (lr, batch_size, num_workers, etc.) that the eval
    # entrypoint doesn't model. Accept and ignore them.
