...
from lda.data.engine import RLBenchDataset
from lda.engine import BaseTrainTester
...
class Arguments(tap.Tap):
    ...
    """ABC = train on tasks A/B/C; ABCD = train on merged ABC+D set (triggers +17870 instr-index shift for D)."""

    # CALVIN-specific: lr decay + scheduler shape
    lr_decay: float = 0.1
    """Multiplier for MultiStepLR at the milestone."""

    # Misc CALVIN flags referenced by train.py / engine
...
class TrainTester(BaseTrainTester):
    ...
    def train_one_step(self, model, criterion, optimizer, step_id, sample, scheduler=None):
        ...
        out, loss_se3, loss_gripper = model(
            sample["trajectory"],
        ...
        if step_id % self.args.accumulate_grad_batches == self.args.accumulate_grad_batches - 1:
            ...
            if scheduler != None:
                ...
                for param_group in optimizer.param_groups:
                        param_group['lr'] = max(param_group['lr'], self.args.end_lr)

        # Log
        if dist.get_rank() == 0 and (step_id + 1) % self.args.val_freq == 0:
            self.writer.add_scalar("lr", optimizer.state_dict()['param_groups'][0]['lr'], step_id)
            self.writer.add_scalar("train-loss/se3_loss", loss_se3, step_id)
            self.writer.add_scalar("train-loss/gripper_loss", loss_gripper, step_id)
    ...
    def evaluate_nsteps(self, model, criterion, loader, step_id, val_iters,
        ...
        for i, sample in enumerate(loader):
            ...
            loss = F.l1_loss(action[..., :7], sample["trajectory"][..., :7].to(device))
            losses.append(loss.unsqueeze(0))

            if dist.get_rank() == 0 and i == 0:
                animate_pose_sequence(
                    action[0].cpu(), sample["trajectory"][0, ..., :7].cpu(),
                    save_path=self.args.log_dir / f"traj_{split}_{step_id}.gif"
                )
            ...
            if step_id > -1:
                if len(losses) > 0:
                    self.writer.add_scalar(
                        "val-loss/traj_l1", torch.cat(losses).mean().item(), step_id
                    )
...
def animate_pose_sequence(pred, gt, save_path=None):
    """
    Animate pose sequences for predicted vs ground truth trajectories.
    
    Args:
        pred: torch tensor [T, 7] with (x, y, z, qx, qy, qz, qw) 
        gt: torch tensor [T, 7] with (x, y, z, qx, qy, qz, qw)
        save_path: if given, saves animation as .gif
    """
    ...
    # Compute reasonable plot limits
    all_pos = np.vstack([pred[:, :3], gt[:, :3]])
    center = all_pos.mean(axis=0)
    span = np.max(np.ptp(all_pos, axis=0))
    margin = max(0.1, span * 0.2)  # At least 10cm margin
    ...
    def plot_coordinate_frame(ax, pos, quat, scale=0.05, label=""):
        """Plot a 3D coordinate frame at given position and orientation."""
        # Convert quaternion to rotation matrix
        # Assuming quaternion format [qx, qy, qz, qw]
        if len(quat) == 4:
            r = R.from_quat(quat)  # scipy expects [x, y, z, w]
        ...
        for i, (axis, color, axis_label) in enumerate(zip(axes, colors, labels)):
            # Rotate axis and translate to position
            rotated_axis = rotation_matrix @ axis
            end_point = pos + rotated_axis
        ...
        if label:
            ax.text(pos[0], pos[1], pos[2] + scale, label, fontsize=10)
    ...
    def update(t):
        ...
        # Plot trajectory history (faded)
        if t > 0:
            # Predicted trajectory history
            ax.plot(pred[:t+1, 0], pred[:t+1, 1], pred[:t+1, 2], 
                   'r--', alpha=0.3, label='Pred trajectory')
            # GT trajectory history  
            ax.plot(gt[:t+1, 0], gt[:t+1, 1], gt[:t+1, 2], 
                   'g--', alpha=0.3, label='GT trajectory')
        
        # Current poses
        pred_pos = pred[t, :3]
        pred_quat = pred[t, 3:7]
        gt_pos = gt[t, :3]  
        gt_quat = gt[t, 3:7]
        
        # Plot current coordinate frames
        plot_coordinate_frame(ax, pred_pos, pred_quat, scale=0.03, label="")
        plot_coordinate_frame(ax, gt_pos, gt_quat, scale=0.03, label="")
        
        # Plot current positions as points
        ax.scatter(pred_pos[0], pred_pos[1], pred_pos[2], 
                  color='red', s=100, label='Predicted')
        ax.scatter(gt_pos[0], gt_pos[1], gt_pos[2], 
                  color='green', s=100, label='Ground Truth')
        
        # Add distance error
        distance_error = np.linalg.norm(pred_pos - gt_pos)
        ax.set_title(f"Timestep {t+1}/{T} | Position Error: {distance_error:.3f}m")
        
        if t == 0:  # Only show legend on first frame
            ax.legend()
