...
class BaseTrainTester:
    ...
    def main(self, collate_fn=default_collate):
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[200000], gamma=self.args.lr_decay)
        # scheduler = None
        ...
        #     repeat=1,
        #     ),
        #     on_trace_ready=torch.profiler.tensorboard_trace_handler('./profiler_log/score'),
        #     record_shapes=True,
        #     with_stack=True
        # ) as profiler:
        for step_id in trange(start_iter, self.args.train_iters):
            ...
            self.train_one_step(model, criterion, optimizer, step_id, sample, scheduler)
            # profiler.step()
