...
from .engine import RLBenchDataset
from .utils import Resize, TrajectoryInterpolator
from lda.utils.calvin import to_relative_action, convert_rotation
...
class CalvinDataset(RLBenchDataset):
    ...
    def __init__(
        ...
        # training split: "ABC" or "ABCD" — controls D-task instruction-index shift
        training_split="ABC",
    ):
        self._cache = {}
        ...
        assert training_split in ("ABC", "ABCD"), training_split
    ...
    def __getitem__(self, episode_id):
        ...
        joints_coords = torch.stack([
            episode[7][i] if isinstance(episode[7][i], torch.Tensor)
            else torch.from_numpy(episode[7][i])
            for i in frame_ids
        ])
        ...
        if self._instructions is not None:
            ...
            # ABCD training: D-task instructions live at indices 0-5124 locally,
            # but the merged instructions array starts D's portion at index 17870
            # (after ABC's 17870 training examples). Shift to address the merged array.
            if task == 'D' and self._training and self._training_split == 'ABCD':
                assert instr_ind < 5124, f"D-task instr_ind {instr_ind} out of range"
                instr_ind = instr_ind + 17870
        ...
        ret_dict = {
            ...
            "joints_coords": joints_coords,  # e.g. tensor (n_frames, num_joints, 7) -> xyz xyzw
        }
        if self._return_low_lvl_trajectory:
            ret_dict.update({
